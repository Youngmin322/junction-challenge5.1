...
> 직접 사용자·어촌계·원전 관계자·관측기관을 새로 모집하거나 인터뷰하는 활동은 제출 전 범위에서 제외한다. 제출 전 검토자는 트랙 제공자·해커톤 관계자, 비구현 팀원, AI 데스크 리서치로 제한한다. 직접 이해관계자 검토 미충족은 숨기지 않되 제출 차단조건으로 쓰지 않고, **사용자 가치·필요 선행시간·운영효과를 입증하지 않았다는 주장 경계**로 보존한다. 최신 실행계획은 [[39) 데이터 부족 항목 전수조사와 수집 실행 결과#T-07 제출까지 12시간 실행표]]를 따른다.
...
| 외부 검토 기록 | Claude·AI 에이전트 검토만 있음 | 제출 전 트랙 제공자 검토와 직접 이해관계자 검토를 분리 | **트랙 제공자 검토 가능 / 직접 관계자 미완료** |
...
| 한울 운전/설비 담당자가 사용자 | **사용자 가설로 보존** | 공식 직무문서로 역할 개연성만 확인했으며, 실제 수신자·승인권자·과업은 제출 후 관계자 검증 전에는 확정할 수 없다. |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| N-P0-01 | P0 | 실제 수신자·검토자·승인자는 누구인가? | 직접 사용자·화면·권한 | 운전·정비·당직 역할이 분리될 것 | 제출 전 공식 직무문서·트랙 제공자 검토 / 실제 역할은 제출 후 인터뷰 | 사용자 가설 고정 전 | **부분 — 직접 검증은 제출 후** | RC-05·08 |
...
| N-P0-09 | P0 | 야간·휴일에는 누가 신호를 받고 확인·준비를 승인하는가? | 실제 사용자·24h workflow | 주간과 다른 승인경로가 있을 것 | 제출 후 당직 RACI·최근 사례 검토 | 운영 workflow 주장 전 | **미지 — 제출 후** | RC-05·08 |
...
| N-P1-03 | P1 | 기존 대응이 잘 작동하는 조건과 실패 조건은 무엇인가? | 실제 공백 | 다층대응은 존재하나 경북 성능은 모름 | 제출 전 Torness·EDF·KHNP desk audit / 현장검토는 제출 후 | 운영효과 주장 전 | **부분** | RC-04·05 |
| N-P1-04 | P1 | 어떤 불확실성 표현이 과신 없이 행동을 돕는가? | UX·경보표현 | 출처·시각·결측·상대순위가 필요할 것 | 역할별 시나리오 과업 테스트 | 디자인+기획 / 데모 전 | **미지; 확률·ETA 금지** | RC-03·08 |
| N-P1-05 | P1 | miss·false alarm·no-data 비용은 누가 부담하는가? | 임계·평가지표 | 역할별 비용이 다를 것 | 제출 전 공개사건 desk scenario / 실제 비용은 제출 후 사용자 검토 | 운영 임계 설계 전 | **미지 — 제출 후** | RC-05 |
...
| RC-05 | 가장 유망한 공백은 외부신호를 다음 현장확인과 준비행동으로 연결하는 구간이다. | 직접 사용자와 현재 병목을 확인하기 전에는 가설이다. | E-11·13·14 | 제출 전에는 트랙 제공자 범위검토, 실제 사용자 검토는 제출 후 Gate |
...
| 단계 | 현재 내용 | 근거 상태 |
| --- | --- | --- |
| 직접 사용자 | 취수·순환수 감시·정비·상황관리 중 실제 승인권자 | 공식 직무문서 기반 가설 — 직접 검증은 제출 후 |
...
| 모순·반례 포함 | 충족 |
| 근거 없는 일반화 방지 | 충족 |
| 외부 전문가·이해관계자 검토 | **직접 관계자 미충족 — 제출 전에는 트랙 제공자 검토로 대체하되 동등한 검증으로 간주하지 않음** |
...
> [!warning] 실행 경계
> 제출 전 12시간에는 Gate 충족 여부와 무관하게 인터뷰 요청, 현장출동, 시설·직원 접촉을 실행하지 않는다. 현재 문서는 제출 후 질문 설계까지만 보존한다.
...
> **국립수산과학원의 해파리 관측·특보와 한울 인근의 공개 해류·기상자료에, 어민·현장 담당자가 제보하거나 연계 API로 전달한 현장 관측정보를 수신·반영해, 한울 취수구 영향권에서 대량 유입을 우선 확인할 해역과 시간대를 근거·불확실성과 함께 선별하고 취수 운영조직의 현장확인과 단계별 준비를 앞당기는 의사결정 지원 서비스.**
...
> **한울 원전의 취수 연속성을 위해, 대량발생 해양생물 가운데 공개 관측과 국내외 시설영향 사례가 가장 풍부한 해파리를 MVP 대상으로 삼아, 국립수산과학원의 관측·특보와 한울 인근의 공개 해류·기상자료 및 제보·연계 API로 수신한 관측정보를 결합하고, 조건부 수송 시나리오상 공개·승인된 한울 취수구 주변 영향권에서 우선 확인할 후보 해역과 시간대를 출처·불확실성과 함께 제시하며, 이후 수신되는 현장 확인 결과를 축적해 취수 운영조직의 단계별 준비와 조기대응 판단을 앞당길 수 있는지 검증하는 상황인지·의사결정 지원 서비스.**
...
3. 제출 후 사용자 검토에서 현행 대응이 충분하다고 나오면 무엇을 포기할 것인가?
...
- 외부 도메인 전문가·직접 이해관계자 검토는 아직 없으며 Investigate Gate의 미충족으로 보존한다. 제출 전에는 트랙 제공자·해커톤 관계자의 문제·데이터 적합성 검토만 받을 수 있고, 이를 실사용자 검증으로 부르지 않는다.
